...
def spider_proxyip():
    try:
        ...
        logging.error("spider_proxyip exception: %s", str(e))
...
def test_proxyip():
    logging.info("proxys before: %d", len(proxys_src))
    threads = []
    try:
        ...
        logging.error(e)
    logging.info("proxys after: %d", len(proxys))
...
def readurl_by_proxy(url):
    try:
        ...
        if source_code.find(b'\xe6\x82\xa8\xe6\x89\x80\xe5\x9c\xa8\xe7\x9a\x84IP') != -1:
            proxys.remove(tet)
            logging.info('proxys remove by IP traffic, new length is: %d', len(proxys))
            return None

    except Exception as e:
        proxys.remove(tet)
        logging.info('proxys remove by exception: %s, proxys new length is: %d', str(e), len(proxys))
        return None
